...
- `autoPatchelfHook` rewrites DT_NEEDED entries in PanGPS and the libwa* libs from their hardcoded `/opt/paloaltonetworks/globalprotect/lib*.so` references to absolute nix-store paths under `$out/opt/...`. The patched binaries are now the **single source of truth** for deployment: `update-env`'s `extractGlobalprotectDebToOptTask` reads `home.file` markers (`~/.local/share/pangp/{source,dest}`, deployed by `contexts/pangp.nix`) and copies `$out/opt/paloaltonetworks/globalprotect/.` into the runtime base. Both binaries land co-located at the runtime base; both are patched (interpreter → nix-store glibc, which exists at that path on NixOS AND Crostini). The earlier wedge — autoPatchelf'd binaries running from `$out/opt/...` while PanGPA's peer ran from `/opt/...` — is closed by aligning everything at the runtime base.
...
- **Working configuration**: place both binaries in the same runtime-base directory (`pangp.passthru.runtimeBase` = `/opt/paloaltonetworks/globalprotect`). They're staged from `pangp.nix`'s autoPatchelf'd `$out/opt/...` at install time via `update-env`'s `extractGlobalprotectDebToOptTask` (marker-driven copy from nix-store to runtime base — see `docs/vpn.md` "NixOS adaptation" subsection). Systemd overrides point `ExecStart` at the runtime-base `PanGPS`/`PanGPA` paths (user unit replicates `HOME` wrapping inline because the makeWrapper script is no longer in the exec path). Both binaries are patched (interpreter → nix-store glibc); co-location preserved by construction. The architecture eliminates the prior duplicate extraction pipeline (`pangp.nix` autoPatchelf vs `update-env` raw `.deb` extract) — `pangp.nix`'s `$out` is now the single source of truth, with `update-env` staging it into the runtime path.
