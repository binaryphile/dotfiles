...
Idempotent. Platform detection: macos -> crostini -> nixos/$HOSTNAME -> debian -> linux. `platform` is injectable via the standard DI pattern (lowercase function variable, overridable by `local` in tests). `stage1TaskGroups` is a pure decision function mapping platform to the set of task groups that stage1 should run (apt, hostname, gpoc, nix, credential). Tested purely without mocking stage1's internals. For post-deployment maintenance, multi-machine sync, and development workflow, see [environment-lifecycle.md](environment-lifecycle.md).
