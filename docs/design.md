...
- **No `autoPatchelfHook`** (and `dontPatchELF = true`). The pangp binaries stay bit-for-bit identical to the .deb-shipped versions for two reasons that overlap but are not the same. (1) The empirically verified gate at IPC time is **directory collocation**: PanGPS reads `dirname(realpath(/proc/<peer-pid>/exe))` and `dirname(realpath(/proc/self/exe))` and requires them to match (see "PanGPS App Integrity check" below). Pristine `.deb` bytes placed at `/opt/.../` for both binaries satisfies the gate by construction. (2) Separately, autoPatchelf-built binaries get rejected with `Error(1322): App Integrity` even when collocated, and the mechanism behind that rejection has NOT been isolated -- autoPatchelfHook rewrites the ELF interpreter path (`.interp`), the `RPATH`/`RUNPATH`, and may touch other bytes; the gate could be any of those (or a real signature check the byte-flip experiment didn't reproduce). The single-axis isolation experiment is tracked at `tasks.dotfiles #26911`. Until that runs, deploying pristine bytes is the working configuration that sidesteps both gates regardless of which Error-1322 mechanism is live. The pristine `$out/opt/...` subtree is the **single source of truth** for deployment: `update-env`'s `extractGlobalprotectDebToOptTask` reads `home.file` markers (`~/.local/share/pangp/{source,dest}`, deployed by `contexts/pangp.nix`) and copies the pristine bytes from nix-store into the runtime base. Both PanGPS and PanGPA land collocated there.
