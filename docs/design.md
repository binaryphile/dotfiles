...
vpn-slice receives three categories of routing arguments:

1. **CIDR ranges** (`10.0.0.0/8`, `172.26.0.0/16`) — route all corporate internal and VPN infrastructure traffic through the tunnel. Eliminates per-host discovery for internal services.
2. **Positional hostnames** (`dm1.devdevicecloud.com`, `gitlab.drm.ninja`, `3.16.193.243`) — AWS-hosted services that resolve correctly via public DNS but whose traffic must traverse the tunnel so the server sees the VPN source IP. vpn-slice adds both routes and `/etc/hosts` entries for these.
3. **`--domains-vpn-dns digi.com`** — split-horizon DNS. Queries for `*.digi.com` are forwarded to the VPN's DNS server, which returns internal IPs (e.g., `stash.digi.com` → `10.9.8.59` instead of the public Atlassian IP). Only domains where internal and external DNS diverge need this; `devdevicecloud.com` and `drm.ninja` resolve identically from both.

`remotemanager.digi.com` is a public Digi site accessed externally — it is intentionally excluded from the tunnel.

The PAC file in `contexts/crostini/home.nix` mirrors this split: `dnsDomainIs(host, ".digi.com")` covers all internal `*.digi.com` hosts (with early `DIRECT` exclusions for public digi sites like `remotemanager.digi.com`), plus an explicit list of the AWS-hosted services.
...
- **`proxy.pac`** is generated by `pkgs.writeText` inside `contexts/crostini/home.nix`. Public digi sites (`remotemanager.digi.com`, `www.digi.com`, `digi.com`) are excluded early as `DIRECT`. Remaining `*.digi.com` hosts are proxied (mirroring vpn-connect's `--domains-vpn-dns digi.com`), plus an explicit list of AWS-hosted VPN services. Everything else returns `DIRECT`.
...
The PAC file mirrors vpn-connect's routing categories: `dnsDomainIs` for `*.digi.com` (covering all internal services automatically), with early `DIRECT` returns for public digi sites (`remotemanager.digi.com`, `www.digi.com`, `digi.com`), plus an explicit list for AWS-hosted services. Adding a new internal `*.digi.com` service requires no changes to either file; adding a new external VPN-routed host requires updating both the vpn-slice positional args and the PAC's `vpnHosts` array; adding a new public `*.digi.com` site requires adding it to the PAC's exclusion list.
