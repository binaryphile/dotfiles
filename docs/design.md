...
- **lib.bash** is sourced from the local repo (`scripts/lib.bash`) when update-env runs from disk. During curl-pipe bootstrap (`curl ... | bash`), `resolveSourceDir` fails and lib.bash is fetched from GitHub branch tip with no verification (same provenance as the outer curl-pipe).
- **task.bash** is sourced from the nix store via `TASK_BASH_LIB` when set (after home-manager switch). When `TASK_BASH_LIB` is unset, task.bash is fetched from a pinned GitHub commit and SHA-256 verified before sourcing. This is not limited to bare-machine first run -- it fires on any run where `TASK_BASH_LIB` is absent: after losing the nix profile, or on platforms without home-manager flake configs. The expected hash is committed in `update-env`. After home-manager switch, `convergeTaskBash` re-sources task.bash from the nix store, replacing the bootstrap copy for the remainder of the run.
...
| `home-manager` (`home.file`) | gitignore_global, tmux.conf, liquidprompt (2), ssh (2), ranger (3) | 9 symlinks (`linux-base.nix`) | Static dotfile configs consumed by programs. No bootstrap dependency. Benefit from HM's atomic generation switching and rollback. |
| `home-manager` (`programs.git`) | .gitconfig | 1 generated file (`linux-base.nix`) | Generated by HM module so `gpg.ssh.program` can interpolate `${pkgs._1password-gui}/bin/op-ssh-sign` (store-bound, host-portable, GC-immune). Migrated 2026-06-24 from a `home.file` symlink whose literal path was wrong on NixOS hosts. |
...
| `update-env` (stage 1) | Base CLAUDE.md: full (`claude/CLAUDE.md`) or thin (`claude/CLAUDE-thin.md`) per `--claude-mode` flag | 1 copy (`claudeBaseCopyTask`) | Copied by update-env rather than HM because stage 2 appends era config, requiring a writable file. HM's store copy is read-only. `--claude-mode thin` deploys the pre-composed thin variant (no `@` imports); stage 2 append tasks are skipped in thin mode. |
| `update-env` (stage 2) | Claude era config (appended to CLAUDE.md) + per-project memory redirects | ~15 files | Era-dependent Claude Code config: `claudeEraConfigTask` appends `CLAUDE-era.md` (era memory instructions, code search, auto-memory prohibition) to the base CLAUDE.md. `claudeMemoryRedirectsTask` installs `era-memory-redirect.md` as `MEMORY.md` in each project's Claude state dir, directing the auto-memory system to era. Requires era to be built and running. All three append tasks (`claudeEraConfigTask`, `claudeTeshtConfigTask`, `claudeTandemConfigTask`) are skipped when `--claude-mode thin` -- thin mode deploys a pre-composed `CLAUDE-thin.md` that inlines equivalent orchestrator-scoped content. |
...
- **Stable configuration** (tmux.conf, ssh/config, liquidprompt theme, ranger rc, etc. -- the bulk of `linux-base.nix`'s `home.file` block) is declared with `source = <tracked flake path>`. Nix snapshots the file into the store at evaluation, the runtime symlink is read-only, accidental writes through `~/X` cannot mutate the working tree, and the configuration is reproducible from any clean checkout. (The historical `gitconfig` entry was migrated to `programs.git` in 2026-06-24 -- see the row in the deployment-owner table above.)
...
**Context-switched files and the `ctxDir` mechanism.** Three files vary per context: `tmux.conf`, `liquidprompt/liquidpromptrc`, `ssh/config`. They cannot be sourced through the top-level `~/dotfiles/<file>` symlinks, because those chain through `~/dotfiles/context/<file>`, and the `context` selector symlink is gitignored -- absent from the flake source snapshot in the nix store, so the chain dangles inside the store and activation fails with `path '...source/context/<file>' does not exist`. The fix is to route per-context paths through a `ctxDir = ./. + "/${contextName}"` prefix in `linux-base.nix`: `source = ctxDir + "/tmux.conf"` resolves to `contexts/${contextName}/tmux.conf`, which is tracked and therefore included in the flake source snapshot. Symlink chains inside the tracked tree are safe; the failure mode occurs only when resolution enters a path excluded from the snapshot (such as the gitignored `context/` selector). `contextName` is supplied via `extraSpecialArgs` in `flake.nix` per `homeConfigurations.<name>` (e.g. `{ contextName = "crostini"; }`). Generalization: flake-path imports may only traverse paths present in the materialized flake snapshot. Context-selection indirections that depend on gitignored or runtime-only paths therefore cannot be used as `home.file` sources -- a tracked selector symlink would be fine; only the excluded-from-snapshot case fails. (Historically `gitconfig` was the fourth ctxDir-routed file; it was migrated to `programs.git` in 2026-06-24 because per-context divergence was an accident, not a requirement, and a Nix-bound `${pkgs._1password-gui}/bin/op-ssh-sign` is host-portable.)

```mermaid
flowchart TD
    N["naive: source = ./../tmux.conf"] --> N1["flake snapshot:<br/>tmux.conf (tracked symlink)"]
    N1 -.->|"target: context/tmux.conf"| N2["context/<br/>(gitignored, absent from snapshot)"]

    F["fix: source = ctxDir + '/tmux.conf'<br/>(ctxDir = ./. + '/' + contextName)"] --> F1["flake snapshot:<br/>contexts/${contextName}/tmux.conf"]
    F1 -->|"tracked symlink"| F2["contexts/desktop/tmux.conf<br/>(real file in snapshot)"]
...
Each platform context can override `home.nix`, `tmux.conf`, and other configs. Top-level files like `home.nix` are symlinks to their context version. (`gitconfig` used to be context-switched too; since 2026-06-24 it is generated by `programs.git` in `linux-base.nix` and applies uniformly to all Linux contexts.)
...
**op-run nix packaging:** `op-run` is built via `mkScriptBin` in `linux-base.nix`. `pkgs._1password-cli` is intentionally absent from its `runtimeInputs` -- the NixOS system wrapper for `op` is used instead of the nix store binary. See the comment in `linux-base.nix`; the system wrapper is managed by nixos-config.

#### Launcher integrity check

Hashed artifacts: `scripts/op-run`, `op-run/projects.bash`, every `op-run/machines/*.allow`. Hashes live in `op-run/checksums` (sha256sum format, committed); verify any time with `( cd ~/dotfiles && sha256sum --check op-run/checksums )`.

Runtime checks:
- *At interactive shell init*, `OpRunIntegrityCheck` (`bash/lib/op-run-integrity.bash`, called from `bash/init.bash`) runs `sha256sum --check op-run/checksums` and verifies `command -v op-run` resolves under `/nix/store/`. Mismatches emit a single warning to stderr; shell startup is not blocked.
- *At commit time*, `.githooks/pre-commit` refuses commits when a hashed file is staged without a matching `op-run/checksums` update. The standard `git commit --no-verify` bypass remains available.

Regen after deliberate edits to any hashed file:
...
`gpd.service` is enabled-at-boot via `home.activation.pangpSystemUnit`'s `systemctl enable` (described below under "pangp packaging"). The post-reboot default is therefore always pangp; `vpn-mode gpoc` is session-scoped and does not survive reboot. The asymmetry is deliberate: gpoc is the broken-upstream client (CVE-2026-0257), so reboot-defaults-to-pangp matches the working configuration. If gpoc eventually becomes the working choice again, `sudo systemctl disable gpd.service` inverts the boot default without removing pangp.
...
- **pangp path**: predicate via bash conditional against captured `globalprotect show --status` output: `[[ $statusOutput == *Connected* && $statusOutput != *"Not Connected"* ]]` (control-plane state). The two-check form excludes a false-positive the naive `grep -q 'Connected$'` regex hits — a line ending in "Not Connected" matches that regex because the literal suffix is "Connected" (bug discovered during #32959 3a testing; the parallel cmdStatus predicate was fixed at `tasks.jeeves #51793`). Already-connected → echo `vpn (pangp) already connected` + return 0. Not-connected (including transient states like `Connecting...` and CLI rc=127) → fall through to `vpn-connect` (which `exec`s the one-shot `globalprotect connect --portal access.digi.com` against `gpd.service`). **No tmux session**. Treating status-CLI failure as "not connected" is intentional: the command's job is to ensure connectivity, not merely observe daemon activity, and a fall-through `globalprotect connect` is idempotent if the tunnel is already up.
...
- **No `autoPatchelfHook`** (and `dontPatchELF = true`). The pangp binaries stay bit-for-bit identical to the .deb-shipped versions because two independent PanGPS-enforced gates both reject autoPatchelf'd binaries (see "PanGPS App Integrity check" below for the long form). (1) The empirically verified gate at IPC time is **directory collocation**: PanGPS reads `dirname(realpath(/proc/<peer-pid>/exe))` and `dirname(realpath(/proc/self/exe))` and requires them to match. Pristine `.deb` bytes placed at `/opt/.../` for both binaries satisfies this gate by construction. (2) A separate uncharacterised rejection fires on autoPatchelf-built binaries even when collocated: `Error(1322): App Integrity`. The mechanism has NOT been isolated -- autoPatchelfHook rewrites the ELF interpreter path (`.interp`), the `RPATH`/`RUNPATH`, and may touch other bytes; the gate could be any of those (or a real signature check the byte-flip experiment didn't reproduce). Single-axis isolation tracked at `tasks.dotfiles #26911`. Deploying pristine bytes sidesteps both gates regardless of which Error-1322 mechanism is live. The pristine `$out/opt/...` subtree is the **single source of truth** for deployment: `update-env`'s `extractGlobalprotectDebToOptTask` reads `home.file` markers (`~/.local/share/pangp/{source,dest}`, deployed by `contexts/pangp.nix`) and copies the pristine bytes from nix-store into the runtime base. Both PanGPS and PanGPA land collocated there.
...
- Systemd unit `gpd.service` is recomposed with `WorkingDirectory=/var/lib/globalprotect` + `StateDirectory=globalprotect` + an `ExecStartPre` (`scripts/gpd-prepare`) that **classifies files in `$out/opt/.../` by writability** and stages them into the StateDirectory accordingly: read-only artifacts (libs, certs, scripts, services) are **symlinked**; runtime-mutable artifacts (`*.log, *.dat, *.txt`) are **copied** as writable files. The cycle that introduced this classification (`#34261`) was prompted by the failure mode where `PanGPS.log` was symlinked into read-only `/nix/store`, silently breaking daemon logging — `gpd-prepare` now actively detects and replaces such symlinks via the predicate `[ -L "$name" ] && [[ $(readlink "$name") == "$NIX_STORE_PREFIX"/* ]]` (`NIX_STORE_PREFIX` defaults to `/nix/store`; the env var is a test affordance). PanGPS writes state files (HipPolicy.dat, HIP_*_Report.dat, PanGpMPR.dat, pangps.xml, *.log, ipt*.txt) to its cwd; the classification ensures those targets are writable while preserving the read-only artifacts as efficient symlinks. The ExecStartPre invariant assumes PanGPS is not running during gpd-prepare's cp operations (current systemd model guarantees this — ExecStartPre runs before ExecStart).

PanGPS App Integrity check (the load-bearing constraint):
- **The verified gate is directory collocation, not signature verification.** At every IPC connection, PanGPS reads `realpath(/proc/<peer-pid>/exe)` and `realpath(/proc/self/exe)` and compares `dirname` of each. Mismatch produces `Error(312): Connected by process not from GP folder` followed by `Error(212): Close socket`. The check makes no reference to the binaries' bytes or signatures. Empirical verification (2026-05-20, Crostini, pangp 6.3.3-638; era memory `c9dfa3f9e2af`): byte-flipping a data byte of the PanGPS binary in place at `/opt/...` (different SHA-384 from pristine) paired with a pristine PanGPA at `/opt/...` is still accepted; relocating a pristine PanGPS to a different directory while PanGPA stays at `/opt/...` produces Error 312.
- **A separate uncharacterised rejection fires on autoPatchelf-built binaries.** Even when collocated at `/opt/...`, autoPatchelf'd binaries are rejected with `Error(1322): App Integrity: Failed to verify PanGPA Signature` (different error code, different log path). The mechanism behind Error 1322 has NOT been isolated. `autoPatchelfHook` modifies the ELF `.interp` section and the binary's `RPATH`/`RUNPATH` in addition to whatever other bytes it touches, so the gate could be (a) a real signature/hash check that a single data-byte flip didn't reproduce, (b) an `.interp` value check, (c) an `RPATH` origin check, or (d) something else. The targeted isolation experiment (apply `patchelf --set-interpreter` only, then `--set-rpath` only, against /opt-collocated binaries) is tracked at `tasks.dotfiles #26911`.
- **Misframing to avoid (corrected 2026-06-03).** A prior cycle (2026-05-27) ran `openssl dgst -sha384 -verify` against pristine vs autoPatchelf'd PanGPA, observed `Verified OK` vs `bad signature`, and concluded "PanGPS performs an asymmetric SHA-384 integrity check on PanGPA." That conclusion was load-bearing but unsupported: the openssl-dgst result only proves the on-disk `.sig` file matches the binary, not that PanGPS uses that signature file at runtime. The associated "9+ hour run with autoPatchelf'd PanGPS not self-rejecting" aside was not independently verifiable. The verified gate is collocation only; the Error-1322 mechanism remains open at #26911.
- **Working configuration**: deploy pristine `.deb` bytes for both binaries into `/opt/paloaltonetworks/globalprotect/`. `pangp.nix` skips autoPatchelfHook to (a) satisfy the collocation gate by construction and (b) sidestep the uncharacterised Error-1322 mechanism that autoPatchelf trips. `programs.nix-ld` provides the loader on NixOS so unmodified-ELF binaries run; Crostini's native FHS provides the loader directly. `pangp.nix`'s pristine `$out` is the single source of truth, with `update-env` staging it into the runtime path on both platforms.
...
- `home.activation.pangpSystemUnit` (opt-in via `services.pangp.enableSystemDaemonOnDebian`, set true in `crostini/home.nix`, false in `desktop/home.nix`) `cp`s the derivation's `gpd.service` to `/etc/systemd/system/` on each `home-manager switch`, then `daemon-reload`s, `systemctl enable`s, and restarts. The `enable` step makes pangp the post-reboot default; without it, every Crostini host-shutdown event (`maitred: Received shutdown request` from ChromeOS sleep or restart) drops the user into implicit-gpoc-mode on next boot, which silently fails since gpoc is broken upstream by CVE-2026-0257. The enable is inside the cmp-gated block (only fires when the unit content changed), but the resulting `multi-user.target.wants/gpd.service` symlink persists across HM switches independently of the gate. Required because home-manager can't directly manage system services on non-NixOS hosts; uses `/usr/bin/sudo` absolute path because home-manager's activation env has minimal PATH.
...
flake.nix wiring: `pangp = import ./pangp.nix { pkgs = linuxPkgs; src = linuxPkgs.requireFile { name = "PanGPLinux-<ver>.tgz"; sha256 = "<base32>"; message = "..."; }; }`. The src is resolved by content hash via `requireFile`, not by absolute filesystem path. `homeManagerFlakeSwitchTask` runs pure: `nix run ~/dotfiles#home-manager -- switch --flake ~/dotfiles#<config>` -- no `--impure` needed. First-time bootstrap on a new machine: download the proprietary tarball from PAN's customer portal, then `nix-store --add-fixed sha256 PanGPLinux-<ver>.tgz` once; thereafter the requireFile lookup succeeds purely on every flake eval. If the tarball isn't staged, the build fails at evaluation with an actionable message rather than silently producing a broken pangp.
...
The shim also injects an in-window operator signal so silent gpoc-vs-pangp dispatch failures (the failure mode that motivated the activation hook's enable-at-boot above) are visible at auth time, not at tunnel-up-fails time. `saml-host-browser` `sed -i`'s `saml.html` to insert a green `position:fixed` banner div ("VPN auth via pangp") after the opening `<body>` tag and wraps PanGPA's auto-submit `<script>` in `setTimeout(...,1000)` so the banner is visible across the range of Okta-response latencies (fast re-auths with valid session cookies would otherwise navigate away from `saml.html` before the banner renders).

The banner is pangp-side only by construction: gpoc's gpauth uses an in-process Rust auth-server that serves the SAML form directly without passing through this shim, so adding a banner to the gpoc path would require either patching gpauth or proxying its auth-server URL. The asymmetry is exploited as a signal: **banner absent during auth = vpn-connect dispatched to gpoc** (and almost certainly silently failed, given gpoc is broken upstream). Operator-facing diagnostic at `docs/vpn.md` "Symptom: VPN widget says down despite completed SAML."

The injection grep-checks for the literal `document.getElementById('myform').submit();` pattern before sed-rewriting. On mismatch (PanGPA format change after a PAN upgrade), the shim logs a stderr warning and serves the unmodified file -- auth proceeds without the banner, the warning surfaces in `journalctl --user -u gpa.service` for follow-up. PanGPA regenerates `saml.html` from scratch every auth cycle, so in-place edits don't accumulate.
...
- **`scripts/probe-lib.bash`** -- shared probe library, sourced by both this repo's panel script AND nixos-config's waybar widget renderer. Caller sets `$State` to its own cache directory before sourcing. The same code path runs on both platforms -- drift in probe semantics is impossible at this layer. Defines:
  - Probe functions: `isStale`, `refresh`, `readState`, `pingHost`, `sshHost`, `combine`, `vpnUp`, `vpnClient`, `bitbucketApiProbe`, `codebergApiProbe`, `digiApiProbe`, `probeReachability`, `probePing`. `vpnUp` returns a bool (used to gate VPN-dependent widgets); `vpnClient` returns the active client identity (`gpoc` / `pangp` / `offline`) for renderers that want to surface which client is up in tooltips. `vpnUp` is defined in terms of `vpnClient` — both move together.
  - Widget metadata tables: `WidgetHost`, `WidgetVpnGated`, `WidgetApiFn`, `WidgetNoSsh` -- single source of truth for host names, VPN gating, API probe selection, and SSH probe skipping. Accessors `widgetHost`, `widgetVpnGated`, `probeWidget` let callers look up by widget key instead of repeating host strings. `WidgetNoSsh` marks hosts that skip the SSH probe -- `probeReachability` checks this table and skips `sshHost` for those entries, and `combine` treats `ssh=skip` + `ping=ok` as "on". **All probe-using widgets are currently in `WidgetNoSsh`** (dm1, nexus, remotemanager, stash, gitlab, codeberg, bitbucket); the SSH probe is therefore inert in production. Reason: `sshHost` performs `ssh -T git@<host>` which triggers a 1Password SSH-agent authorization prompt on every poll (every 600s x N hosts), making the desktop unusable. The `sshHost` code remains intact and the `WidgetNoSsh` opt-out is keyed per-widget so individual hosts can be re-enabled later if SSH becomes the right reachability signal AND the 1P agent path can be made non-interactive.
...
- SSH probe (`sshHost`): every 600s. `ssh -T git@<host>`; rc 0/1 or "shell request failed" both count as ok. **Currently inert** -- all widgets are in `WidgetNoSsh` (see above) because the probe triggered 1Password SSH-agent prompts on every poll.
- TCP/443 ping (`pingHost`): every 30s. Uses `bash`'s `/dev/tcp/<host>/443` rather than ICMP because most vendor sites block ICMP. **This is the operative reachability signal** for all widgets in `WidgetNoSsh` (= all of them, post-SSH-disable): `combine` returns `on` when `ping=ok && api=on && ssh=skip`, so the displayed state is driven by ping + API status alone.
...
**VPN gating**: `dm1`, `stash`, `gitlab`, `nexus` modules return early (empty string) when `vpnUp` is false — neither `tun0` (gpoc) nor `gpd0` (pangp) has `LOWER_UP` set in its netlink flag set — the segment vanishes from the bar entirely, since tmux's per-segment range tolerates empty content. The carrier-up check (rather than interface existence or textual `state UP`) is load-bearing: tun-class devices report `state UNKNOWN` even when carrying traffic (no carrier-sense layer), and both `tun0` and `gpd0` persist in `state DOWN` after disconnect on their respective clients, so presence alone would falsely register as "VPN up". (Originally added in `panel + probe-lib: require state=UP`, alongside the off->on transition hook that invalidates the SSH cache so widgets don't show stale `fail` for the cache TTL after VPN comes back; later refined from `state UP` to `LOWER_UP` to correctly match pangp's `gpd0` interface.) `remotemanager` is public (not VPN-gated) and always probed.
