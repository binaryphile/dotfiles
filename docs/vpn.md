...
Result: `gpd.service` runs PanGPS from the nix store, `WorkingDirectory=/var/lib/globalprotect` (writable, systemd-`StateDirectory`-managed). `ExecStartPre` (`scripts/gpd-prepare`) classifies files by writability — symlinks read-only artifacts (libs, certs, scripts), copies runtime-mutable artifacts (`*.log, *.dat, *.txt`) as writable, and actively replaces nix-store-targeted symlinks for the runtime-mutable set (`#34261`); see `design.md` §"VPN (UC-7, UC-7a)" for the full classification rationale. PanGPA runs as a home-manager user-service, with `HOME` wrapped to `$HOME/.local/share/globalprotect` so it doesn't pollute the top-level home dir with `GP_HTML/` and `.GlobalProtect/`.
