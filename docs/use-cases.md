...
- **Extensions:**
  ...
  - *Audit fallback log grows past size cap:* When the file at `${XDG_STATE_HOME:-~/.local/state}/op-run/audit.log` exceeds `AuditLogMaxBytes` (default 1 MiB), the next fallback write rotates it to `audit.log.1` (overwriting any prior rotation) and the new `audit.log` opens with a `{"event":"rotated","at":"<ISO8601>","prev_size":<bytes>}` marker. `AuditLogMaxBytes=0` disables the cap (debugging escape hatch). Disk use bounded at ~2× the cap. Detective posture preserved: rotation failures (mv error, jq missing, marker write error) never block a launch — the function returns 0 on any failure path.
...
| UC-11 Use a Credentialed Tool | v1 + v2 implemented (mcp-atlassian: Bitbucket + Confluence + Jira) | Bash launcher op-run wrapping `op run`; project registry in dotfiles (path-keyed); machine allowlist per host. Tamper-evident launcher hash check (`op-run/checksums` + `OpRunIntegrityCheck` at interactive-shell init + pre-commit drift gate). Audit-log fallback size-cap rotation (default 1 MiB; single rotation to `audit.log.1`; rotation marker; `AuditLogMaxBytes` override). Generalized failure-mode probing closed-by-decision 2026-05-30 (`#2908`): v1's exit-67 passthrough of `op` CLI's native stderr is operator-actionable for the empirically-observed failure classes; expanding substring matching would add brittleness without commensurate benefit until 1Password ships structured error output. |
