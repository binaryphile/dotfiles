...
## Integrity check (UC-11 detective control)
...
- **At interactive-shell init**, `OpRunIntegrityCheck` (`bash/lib/op-run-integrity.bash`, called from `bash/init.bash`) runs `sha256sum --check op-run/checksums` and verifies that `command -v op-run` resolves under `/nix/store/`. Mismatches emit a single warning to stderr; shell startup is not blocked (control is Detective per `~/projects/jeeves/security/threat-model.md`).
